  ...
  */
            ...
            steps {
                script {
                    ...
                    withCredentials([usernamePassword(credentialsId: 'hub-docker', passwordVariable: 'PASSWORD', usernameVariable: 'USERNAME')]) {
                        sh "docker build -t esso4real/myhtmlapp:Tek-Experts-2.4.4 ."
                        sh "echo $PASSWORD | docker login -u $USERNAME --password-stdin"
                        sh "docker push esso4real/myhtmlapp:Tek-Experts-2.4.4"
                            }
                        }
                    }
                }
         ...
         stage('Deploy App') {
            steps {
                script {
                    echo "Deploying App"
                    def dockerCmd = "docker run -d esso4real/myhtmlapp:Tek-Experts-1.0"
